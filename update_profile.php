<?php 
    ...
    if (isset($_POST['update_profile'])) {
        ...
        $update_nm = mysqli_query($conn, "UPDATE users_form SET name = '$update_name'
                                            WHERE user_id = '$user_id'");
        if ($update_nm) {
            $alert[] = "Name updated succesful!"; 
        }
            if(filter_var($update_email, FILTER_VALIDATE_EMAIL)){
                $update_email = mysqli_query($conn, "UPDATE users_form SET email = '$update_email'
                                            WHERE user_id = '$user_id'");
            }else {
                $alert[] = "$update_email is not valid email";
            }
        ...
        if (!empty($image)) {
            if($image_size > 2000000){
                ...
                $update_img = mysqli_query($conn, "UPDATE users_form SET img = '$image'
                                            WHERE user_id = '$user_id'");
        ...
        if (!empty($old_pass) || !empty($new_pass) || !empty($confirm_pass)) {
            if($old_pass != $main_pass){
                ...
                $update_pass = mysqli_query($conn, "UPDATE users_form SET password = '$confirm_pass'
                                            WHERE user_id = '$user_id'"); 
...
<head>
    ...
    <title>JUXY | Update Profile</title>
